...
- `--log-pipeline-status` writes safe Native Core pipeline timing diagnostics to stderr only for Phase 6.5 backend evaluation. Each compact `[pipeline] periodic:` line includes emitted frame count plus capture, preprocess, tracking, write, and total frame durations. In `--realtime` mode, `writeDurationMs` includes the stdout flush cost before pacing; realtime sleep/pacing remains outside `totalFrameDurationMs`. Stdout remains newline-delimited MotionFrame JSON only.
- `--pipeline-status-interval N` writes periodic pipeline diagnostics every `N` emitted frames when `--log-pipeline-status` is also set. `N` must be between 1 and 100000. If omitted, pipeline diagnostics use the safe default interval of 60 emitted frames.
