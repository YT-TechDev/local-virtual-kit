...
The approval-gated local build spike evidence is recorded in `docs/MEDIAPIPE_FACE_LANDMARKER_RESEARCH.md#c--bazel-local-build-spike-2026-06-17`. Bazelisk 1.29.0 was installed via winget. MediaPipe source was shallow-cloned into the scratch directory outside the LVK repository (`C:\Users\Dev\Developments\lvk-mediapipe-cpp-build-spike\mediapipe`, 72 MB). The smallest Face Landmarker-related Bazel target (`face_landmarker_result`) was inspected. The minimal Bazel build probe was attempted once. Bazel 7.4.1 downloaded and the server started. The probe did not complete: it stopped at the first actionable blocker — TensorFlow's Python hermetic version detection (`python_version_repo`) could not locate system Python during the repository fetch phase, before reaching C++ compilation. MSYS2 was not installed. Java was not installed. No retry or fix attempt was performed. No broader FaceLandmarker target was built. No model/task files were downloaded. No camera access occurred. No LVK source, runtime, or build files were changed. The C++/Bazel route remains unvalidated. A future PR may either run a focused Python/Bazel environment fix-and-reprobe, or start a helper-process architecture decision PR; neither is done here. MediaPipe Face Landmarker remains a candidate only. No tracking backend is selected. No dependency is added to LVK. No model/task file is committed or approved for bundling. No MotionFrame schema change is made. No camera/webcam validation was performed.

### Helper-process architecture memo (2026-06-17)
...
### Helper-process prototype design (2026-06-17)

A design-only helper-process prototype design memo is recorded in `docs/TRACKING_HELPER_PROCESS_PROTOTYPE_DESIGN.md`, following the pivot away from further immediate C++ / Bazel toolchain chasing. It defines a Native Core-owned helper-process prototype boundary, staged phases (H0 design → H1 non-camera synthetic prototype → H2 future local-frame-access design → H3 production), an illustrative Native Core-only internal result shape, lifecycle/backpressure/crash rules, safe diagnostics, and privacy/security gates. It is design-only: no backend is selected, no dependency/IPC is added, raw frame IPC is not approved, no model/task bundling is approved, and no MotionFrame schema change is made. The recommended next step is a small, separate, non-camera H1 implementation spike only after the design is reviewed.
