...
- For successful Electron native runtime lifecycle actions, verify local status feedback such as `Native runtime started.` after start and `Native runtime stopped.` after stop; this success feedback should not appear while a lifecycle action is pending, and stale feedback should disappear when the relevant native tracker status changes.
