...
requireMatch(
  source,
  /<div\s+className=['"]diagnostics-preview['"]\s+aria-labelledby=['"]diagnostics-preview-label['"]\s+aria-label=['"]Native runtime diagnostics preview['"][\s\S]*?<strong\s+id=['"]diagnostics-preview-label['"]\s+className=['"]status-detail-label['"]>\s*Diagnostics preview\s*<\/strong>[\s\S]*?<pre>\s*\{nativeRuntimeDiagnostics\}\s*<\/pre>/u,
  'native runtime diagnostics preview must keep a visible label, expose aria-label="Native runtime diagnostics preview", and show the exact copied diagnostics text',
);
requireMatch(
  source,
